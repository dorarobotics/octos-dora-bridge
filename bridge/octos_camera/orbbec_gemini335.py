...
class Gemini335Camera:
    ...
    Importing this module does not import pyorbbecsdk. The native SDK is loaded
    only when opening or reading the camera.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if color_size is None:
            # Allow overriding the color resolution via env (e.g. "848x480") so
            # it can be matched to the hand-eye calibration without code edits.
            env = os.environ.get("OCTOS_CAMERA_COLOR_SIZE")
            if env and "x" in env.lower():
                w, h = env.lower().split("x", 1)
                color_size = (int(w), int(h))
            else:
                color_size = (1280, 720)
    ...
    def open(self) -> None:
        ...
        # Use uncompressed RGB, not MJPG: at 1280x720 with SW alignment the
        # MJPG stream yields frames that cv2 cannot decode. RGB is reliable and
        # USB3.2 has ample bandwidth (720p RGB @30fps ~= 79 MB/s).
        color_profile = _video_profile(
            color_profiles,
            width=self.color_size[0],
            height=self.color_size[1],
            fmt=obsdk.OBFormat.RGB,
            fps=self.fps,
        )
        config.enable_stream(color_profile)

        # Software depth-to-color alignment inside the pipeline. Hardware D2C
        # (HW_MODE) is unreliable on this unit (firmware accepts it but yields
        # no aligned frames), so we use SW_MODE. The aligned depth is resampled
        # to the color resolution, so the color intrinsics apply to both.
        config.set_align_mode(obsdk.OBAlignMode.SW_MODE)
        ...
        try:
            ...
            pass
        # Color exposure. Honor OCTOS_CAMERA_EXPOSURE (manual) when set — useful
        # in dim rooms where auto-exposure caps too low to see anything;
        # otherwise enable auto-exposure and clear any stale manual value left on
        # the device by tooling.
        try:
            manual_exp = os.environ.get("OCTOS_CAMERA_EXPOSURE")
            if manual_exp:
                device.set_bool_property(obsdk.OBPropertyID.OB_PROP_COLOR_AUTO_EXPOSURE_BOOL, False)
                device.set_int_property(obsdk.OBPropertyID.OB_PROP_COLOR_EXPOSURE_INT, int(manual_exp))
            else:
                device.set_bool_property(obsdk.OBPropertyID.OB_PROP_COLOR_AUTO_EXPOSURE_BOOL, True)
        except Exception:
            pass
        try:
            ...
            self._depth_filter_list = None
        # Warm up: drop the first frames so color auto-exposure / white balance
        # converge before the first read. Without this the initial frames are
        # badly underexposed and color detection (HSV masks) finds nothing.
        for _ in range(self.warmup_frames):
            try:
                if pipeline.wait_for_frames(200) is None:
                    continue
            except Exception:
                break
    ...
    def read(self, timeout_ms: int = 3000) -> RgbdFrame:
        ...
        if self._pipeline is None:
            self.open()
        # Frames are already depth-to-color aligned by the pipeline (SW_MODE).
