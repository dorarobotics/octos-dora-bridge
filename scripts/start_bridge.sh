  ...
  cat > "$manifest" <<EOF
{
  ...
  "arm_home": [-0.11737818705720107, -1.4660765716752369, 0.2961305895691478, 1.4185115546978089, -0.02301533079552962],
  "signs": [1, 1, 1, 1, 1],
  "offsets": [0, 0, 0, 0, 0],
  "grip_open_w": 0.06,
  "grip_open_pct": 100.0,
...
export LD_LIBRARY_PATH="$extra_pythonpath:\${LD_LIBRARY_PATH:-}"
if [ "\$#" -eq 1 ]; then
  case "\$1" in
    "-m "*)
      _module="\${1#-m }"
...
find_rebot_hw_repo() {
  local candidates=(
    "${REBOT_HW_NODE:-}"
    "$BRIDGE_REPO/../rebot-hw-dora-node"
    "$HOME/.octos/skills/skills/rebot-hw-dora-node"
  )
  local d
  for d in "${candidates[@]}"; do
    [ -n "$d" ] || continue
    if [ -f "$d/manifests/so101-hw.json" ] && [ -d "$d/rebot_hw_node" ]; then
      cd "$d" && pwd
      return 0
    fi
  done
...
resolve_so101_dataflow() {
  local template="$BRIDGE_REPO/dataflows/so101-hw-bridge.yaml"
  [ -f "$template" ] || {
    echo "missing SO101 dataflow template: $template" >&2
    exit 1
  }
  ...
  local rebot_hw_repo
  rebot_hw_repo="$(find_rebot_hw_repo)" || {
    echo "rebot-hw-dora-node is required; set REBOT_HW_NODE=/path/to/rebot-hw-dora-node" >&2
    exit 1
  }

  local venv_py="${SO101_VENV_PYTHON:-${ADORA_VENV_PYTHON:-$RUN_DIR/venv-python}}"
  if [ ! -x "$venv_py" ]; then
    local py
    py="$(find_default_python)" || {
      echo "could not find python with SO101 bridge dependencies; set SO101_VENV_PYTHON=/path/to/python-or-wrapper" >&2
      exit 1
    }
    local extra_pythonpath="${SO101_EXTRA_PYTHONPATH:-$(default_extra_pythonpath "$py")}"
    local moveit_arm_repo="$BRIDGE_REPO/../moveit-arm-dora-node"
    [ -d "$moveit_arm_repo/moveit_arm_node" ] || moveit_arm_repo="$HOME/.octos/skills/skills/moveit-arm-dora-node"
    cat > "$venv_py" <<EOF
#!/usr/bin/env bash
export PYTHONPATH="$extra_pythonpath:$BRIDGE_REPO/bridge:$moveit_arm_repo:$rebot_hw_repo:$dora_moveit2:$dora_moveit2/examples:$dora_moveit2/examples/move_group_demo:\${PYTHONPATH:-}"
export LD_LIBRARY_PATH="$extra_pythonpath:\${LD_LIBRARY_PATH:-}"
if [ "\$#" -eq 1 ]; then
  case "\$1" in
    "-m "*)
...
EOF
  ...
  local resolved="$RUN_DIR/so101-hw-bridge.yml"
  sed -e "s|path: ./venv-python|path: $venv_py|g" \
      -e "s|\${DORA_MOVEIT2}|$dora_moveit2|g" \
      -e "s|\${REBOT_HW_NODE}|$rebot_hw_repo|g" \
      "$template" > "$resolved"
  DATAFLOW="$resolved"
}

if [ "${OCTOS_ROBOT:-}" = "so101" ] && [ -z "${SO101_BRIDGE_MODE:-}" ] && [ -z "${ADORA_BRIDGE_MODE:-}" ]; then
  SO101_BRIDGE_MODE=so101-hw
fi
...
for _ in $(seq 1 60); do
  if control_plane_alive &&
    ...
    ps -eo pid=,args= | awk '
      ...
      !/awk/ {print $1}
    ' > "$RUN_DIR/node.pids"
    echo "$BRIDGE_LABEL bridge ready at http://$BRIDGE_HTTP_HOST:$BRIDGE_HTTP_PORT (name=$NAME)"
    exit 0
  fi
  sleep 1
done
